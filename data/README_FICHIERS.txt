...
# Détail des fichiers du dossier data

---

## 📊 Tableau récapitulatif

| Fichier                | Description détaillée                           | Format      | Mise à jour | Exemple de contenu |
|------------------------|------------------------------------------------|-------------|-------------|-------------------|
| programs.json          | Liste complète des applications et outils      | JSON        | Automatique | {"apps": [...]}   |
| config.json            | Paramètres globaux de l’application           | JSON        | Manuel      | {"theme": "dark"}|
| office_links.json      | Liens vers suites bureautiques                 | JSON        | Manuel      | {"office": {...}} |
| programs_backup.json   | Sauvegarde/restauration de la base             | JSON        | Script      | {"backup": [...]} |
| theme_config.json      | Thèmes graphiques et couleurs                  | JSON        | Manuel      | {"colors": {...}} |
| RAPPORT_EXPANSION.md   | Historique des ajouts/suppressions             | Markdown    | Manuel      | ## Ajout V17      |
...
### programs.json
Base principale : contient toutes les applications et outils référencés dans NiTriTe. Mise à jour automatique lors de l’ajout/suppression via l’interface.

### config.json
Paramètres globaux : thème, préférences utilisateur, options d’affichage. Modifiable manuellement ou via l’application.

### office_links.json
Références vers les suites bureautiques (MS Office, LibreOffice, etc.), pour accès rapide et automatisation.

### programs_backup.json
Sauvegarde/restauration : utilisé par les scripts pour restaurer la base en cas de problème ou de corruption.

### theme_config.json
Définit les couleurs, polices et thèmes graphiques de l’application. Permet la personnalisation avancée.

### RAPPORT_EXPANSION.md
Historique des évolutions : chaque modification majeure ou ajout d’application doit être documenté ici.

---

## 📝 Conseils

- Sauvegardez avant toute modification manuelle
- Vérifiez la validité des fichiers JSON (syntaxe)
- Documentez les évolutions dans RAPPORT_EXPANSION.md

---

**Pour toute question, consultez le README.md du dossier data ou ouvrez une issue sur GitHub.**
