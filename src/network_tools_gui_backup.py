...
class NetworkToolsGUI(ctk.CTkFrame):
    ...
    def create_ui(self):
        ...
        self.connections_tab = tk.Frame(self.notebook, bg=self.colors['bg'])
        ...
        self.ports_tab = tk.Frame(self.notebook, bg=self.colors['bg'])
        self.notebook.add(self.ports_tab, text="  🔐 Ports  ")
        self._create_ports_tab()

        # Onglet 5: Test de vitesse
        self.speed_tab = tk.Frame(self.notebook, bg=self.colors['bg'])
    ...
    def _create_info_tab(self):
        ...
        container = tk.Frame(self.info_tab, bg=self.colors['bg'])
        ...
        # IP publique
        tk.Frame(container, bg=self.colors['bg'], height=20).pack()
    ...
    def _create_connections_tab(self):
        ...
        container = tk.Frame(self.connections_tab, bg=self.colors['bg'])
        container.pack(fill=tk.BOTH, expand=True, padx=20, pady=20)
        ...
        self.conn_count_label = tk.Label(
            header,
            text="0 connexions",
            bg=self.colors['secondary'],
            fg=self.colors['primary'],
        ...
        # Filtres
        filter_frame = tk.Frame(container, bg=self.colors['bg'])
        ...
        tree_frame = tk.Frame(container, bg=self.colors['bg'])
    ...
    def _create_scanner_tab(self):
        ...
        container = tk.Frame(self.scanner_tab, bg=self.colors['bg'])
        container.pack(fill=tk.BOTH, expand=True, padx=20, pady=20)

        # Configuration
        config_frame = tk.Frame(container, bg=self.colors['secondary'], relief=tk.FLAT)
        config_frame.pack(fill=tk.X, pady=10, padx=5)
        config_frame.config(highlightbackground=self.colors['border'], highlightthickness=1)
        ...
        # Réseau à scanner
        input_frame = tk.Frame(config_frame, bg=self.colors['secondary'])
    ...
    def _create_ports_tab(self):
        ...
        container = tk.Frame(self.ports_tab, bg=self.colors['bg'])
        container.pack(fill=tk.BOTH, expand=True, padx=20, pady=20)
    ...
    def _create_speed_tab(self):
        ...
        container = tk.Frame(self.speed_tab, bg=self.colors['bg'])
        container.pack(fill=tk.BOTH, expand=True, padx=20, pady=20)
        ...
        # Status
        self.speed_status_label = tk.Label(
            container,
        ...
        # Résultats
        results_frame = tk.Frame(container, bg=self.colors['secondary'], relief=tk.FLAT)
        results_frame.pack(fill=tk.BOTH, expand=True, pady=20, padx=5)
        results_frame.config(highlightbackground=self.colors['border'], highlightthickness=1)
    ...
    def _create_speed_result_widget(self, parent, title):
        ...
        frame = tk.Frame(parent, bg=self.colors['secondary'])
    ...
    def _load_info_thread(self):
        ...
        try:
            ...
            def update_ui():
                ...
                self.dns_label.config(text=", ".join(dns_servers) if dns_servers else "Non disponible")
                self.public_ip_label.config(text=public_info.get('ip', 'Non disponible'))
                self.location_label.config(text=location if location != ", " else "Non disponible")
                self.isp_label.config(text=public_info.get('org', 'Non disponible'))
    ...
    def _refresh_connections_thread(self):
        ...
        try:
            ...
            def update_ui():
                ...
                self.conn_count_label.config(text=f"{len(connections)} connexions")
    ...
    def _network_scan_thread(self, network):
        ...
        try:
            ...
            def progress(current, total, host):
                ...
                self.after(0, lambda: self.scan_progress.config(value=percent))
                self.after(0, lambda: self.scan_status_label.config(text=f"Scan en cours... {current}/{total} hosts"))
            ...
            def update_results():
                ...
                self.scan_status_label.config(text=f"Scan terminé - {len(results)} appareils trouvés")

            self.after(0, update_results)

        except Exception as e:
            self.logger.error(f"Erreur scan réseau: {e}")
            self.after(0, lambda: self.scan_status_label.config(text=f"Erreur: {e}"))
        finally:
            self.is_scanning = False
            def reset_ui():
                self.scan_btn.config(state=tk.NORMAL, text="🔍 Démarrer le Scan")
    ...
    def start_port_scan(self):
        ...
        self.port_scan_btn.config(state=tk.DISABLED, text="⏳ Scan en cours...")
    ...
    def _port_scan_thread(self, host):
        ...
        try:
            def progress(current, total, port):
                percent = (current / total) * 100
                self.after(0, lambda: self.port_progress.config(value=percent))
                self.after(0, lambda: self.port_status_label.config(text=f"Scan port {port}... {current}/{total}"))
            ...
            def update_results():
                ...
                self.port_status_label.config(text=f"Scan terminé - {len(results)} ports ouverts")

            self.after(0, update_results)

        except Exception as e:
            self.logger.error(f"Erreur scan ports: {e}")
            self.after(0, lambda: self.port_status_label.config(text=f"Erreur: {e}"))
        finally:
            def reset_ui():
                self.port_scan_btn.config(state=tk.NORMAL, text="🔐 Scanner Ports Communs")
                self.port_progress['value'] = 0
            self.after(0, reset_ui)

    def start_speed_test(self):
        """Démarre le test de vitesse"""
        self.speed_test_btn.config(state=tk.DISABLED, text="⏳ Test en cours...")
    ...
    def _speed_test_thread(self):
        ...
        try:
            def progress(step, message):
                self.after(0, lambda msg=message: self.speed_status_label.config(text=msg))

            results = self.network_manager.test_internet_speed(progress_callback=progress)

            def update_results():
                if results['success']:
                    self.download_speed_label.config(text=f"{results['download_mbps']:.1f} Mbps")
                    self.upload_speed_label.config(text=f"{results['upload_mbps']:.1f} Mbps")
                    self.ping_label.config(text=f"{results['ping_ms']:.0f} ms")
                    self.speed_status_label.config(text="✅ Test terminé avec succès")
                else:
                    self.speed_status_label.config(text=f"❌ Erreur: {results.get('error', 'Inconnue')}")

            self.after(0, update_results)

        except Exception as e:
            self.logger.error(f"Erreur test vitesse: {e}")
            self.after(0, lambda: self.speed_status_label.config(text=f"❌ Erreur: {e}"))
        finally:
            self.after(0, lambda: self.speed_test_btn.config(state=tk.NORMAL, text="⚡ Lancer le Test"))
